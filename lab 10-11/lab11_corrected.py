...
def insert_or_update_user(name, phone_number):
    ...
    cursor.execute("SELECT score FROM phone WHERE user_name = %s", (name,))
    existing = cursor.fetchone()

    if existing:
        print(f"⚠ Контакт с именем '{name}' уже существует. Текущий номер: {existing[0]}")
        choice = input("Хотите обновить номер? (y/n): ").strip().lower()
        if choice == 'y':
...
def show_all_contacts():
    cursor.execute("SELECT user_id, user_name, score FROM phone ORDER BY user_id")
    rows = cursor.fetchall()
    if not rows:
        print("\n📭 Контактов нет.")
    else:
...
if __name__ == '__main__':
    try:
        ...
        if op == "1":
            ...
            phone = input("Телефон (только цифры): ").strip()
